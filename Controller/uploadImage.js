...
// Improved rate limiting handler with concurrency tracking
const MAX_CONCURRENT_REQUESTS = 5; // Maximum concurrent requests to Printful
...
// Enhanced printful client with queue system
const queuedPrintfulClient = (config) => {
...
printfulClient.interceptors.response.use(
  ...
  async (error) => {
    if (error.response && error.response.status === 429) {
      const retryAfter =
        parseInt(error.response.headers["retry-after"] || "3") * 1000;
      console.log(
...
// Group products by priority to allow batch processing
const productGroups = {
  highPriority: [223, 206], // T-Shirt and Hat - most common products
  mediumPriority: [821, 644, 788], // Hoodie, Jersey, Water Bottle
  lowPriority: [605, 682, 660, 565], // AirPods, Notebooks, Buttons, etc.
};
...
const categorizeProducts = (products) => {
  return products.reduce(
    (acc, product) => {
      if (productGroups.highPriority.includes(product.product_id)) {
        acc.high.push(product);
      } else if (productGroups.mediumPriority.includes(product.product_id)) {
...
const productConfigs = [
  // Product configs remain the same as in the original code
  {
    product_id: 223,
...
// Create a cache for mockup task results to avoid redundant API calls
const mockupTaskCache = new Map();

// Optimized mockup URL fetching with smart polling
const getMockupUrl = async (taskKey) => {
  // Check cache first
  if (mockupTaskCache.has(taskKey)) {
    return mockupTaskCache.get(taskKey);
  }

  const maxAttempts = 10;
  let waitTime = 500; // Start with 0.5 second (reduced from 1s)
  const maxWaitTime = 5000; // Max 5 seconds between attempts (reduced from 8s)

  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      const response = await queuedPrintfulClient({
        method: "get",
        url: `/mockup-generator/task?task_key=${taskKey}&store_id=${PRINTFUL_STORE_ID}`,
      });

      if (response.data.result.status === "completed") {
        const url = response.data.result.mockups[0].mockup_url;
        mockupTaskCache.set(taskKey, url); // Cache the result
        return url;
      }

      if (response.data.result.status === "failed") {
        console.error("Mockup generation failed:", response.data.result);
        mockupTaskCache.set(taskKey, null); // Cache the failure
        return null;
      }

      // Dynamic backoff based on attempt number
      const progressFactor = Math.min(((attempt + 1) / maxAttempts) * 2, 1);
      waitTime = Math.min(waitTime * (1 + 0.5 * progressFactor), maxWaitTime);
  ...
  mockupTaskCache.set(taskKey, null); // Cache the timeout
  return null;
};

// Process mockup for each product in batch
const processMockupBatch = async (configs, imageUrl) => {
  const batchPromises = configs.map(async (config) => {
    try {
      ...
      const response = await queuedPrintfulClient({
        method: "post",
        url: `/mockup-generator/create-task/${config.product_id}?store_id=${PRINTFUL_STORE_ID}`,
        data: mockupConfig,
      });
...
// 2. Generate Mockups API - Optimized with priority scheduling
const generateMockups = async (req, res) => {
  try {
    ...
    // Categorize products by priority
    const categorizedProducts = categorizeProducts(productConfigs);

    // Process high priority products first (most important)
    const highPriorityResults = await processMockupBatch(
      categorizedProducts.high,
      imageUrl
    );

    // Start processing medium and low priority in parallel (non-blocking)
    const mediumPriorityPromise = processMockupBatch(
      categorizedProducts.medium,
      imageUrl
    );
    const lowPriorityPromise = processMockupBatch(
      categorizedProducts.low,
      imageUrl
    );

    // Combine all results
    const [mediumPriorityResults, lowPriorityResults] = await Promise.all([
...
// Optimized mockup results fetching with parallel processing
const getMockupResults = async (req, res) => {
  try {
    ...
    // Batch process requests with concurrency control
    const batchSize = 5; // Process 5 mockups at a time
    const results = [];
    ...
    const successfulMockups = results.filter((m) => m.success);
    const failedMockups = results.filter((m) => !m.success);
